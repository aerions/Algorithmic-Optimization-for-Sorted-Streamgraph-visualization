...
function streamGraph(svg) {
    ...
    function calculate_layout(layers) {
      ...
      // console.log(layers)
        ...
        timeBlocks.map((arr, ind1) => {
            ...
            arr.sort((a, b) => (less(a[1], b[1], ind1) ? -1 : 1));
        ...
        const mx = d3.max(stackSum, d => d[layers.length-1]);
        console.log(mx);

        timeBlocks.forEach((arr, ind1) => {
            const offset = (mx - stackSum[ind1][layers.length - 1]) / 2;
            arr.forEach((d, ind2) => {
    ...
    let stream_graph = function (source_data, options) {
        ...
        if (calculate === calculate_layout && false) {
        ...
        for(let i = 0; i < data.length; i++) {
            const ts = data[i];
            for(let j = 0; j < ts.length - 2; j+=2) {
                if(ts[j].y < ts[j+2].y) {
                  if(ts[j].y0 + ts[j].y > ts[j+2].y0) {
                    errorSum += 1;
                  }
                } else if(ts[j].y > ts[j+2].y) {
                  if(ts[j].y0 < ts[j+2].y0 + ts[j+2].y) {
                      errorSum += 1;
                  }
                } else {
                  // if(ts[j].y0 !== ts[j+2].y0) {
                  //     errorSum += 1;
                  // }
                }
            }
...
function calculateLayoutByDynamic(layers) {
    ...
    */
    ...
    const maxHeight = Math.floor(1.1 * d3.max(heights, d => d3.sum(d))) + (heights[0].length) *PADDING;
    // const maxHeight = 100;
    ...
    function dp(H1, H2, lx, ly, rev = 0) {
        ...
        function doLeft() {
            ...
            for (let i = 0; i + xHeight + PADDING <= H1; i++) {
                let err = 0;
                if (condition === 0 && x > ind1InRight && H1 < minH + heights[1][ind1InRight]) {
                    err = 1
                }
                if (condition === 1 && x < ind1InRight && H1 - i > maxH) {
                    err = 1
                }
                ...
                const res = err + dp(nextH, H2, x, ly, rev ^ 1);
                if (f[H1][H2][lx][ly] > res || (f[H1][H2][lx][ly] === res && Math.random() > 0.5)) {
                    f[H1][H2][lx][ly] = res;
        ...
        function doRight() {
            ...
            for (let i = 0; i + yHeight + PADDING <= H2; i++) {
                let err = 0;
                // if (condition === 0 && x > ind1InRight && H1 - i < minH + heights[1][ind1InRight]) {
                if (condition === 0 && x > ind1InRight && (H1 - i < minH || H1 < minH + heights[heights[1][ind1InRight]]) ) {
                    err = 1
                }
                if (condition === 1 && x < ind1InRight && (H1 > maxH + heights[heights[1][ind1InRight]] || H1 - i > maxH)) {
                    err = 1
                }
                ...
                const res = err + dp(H1, nextH, lx, y, rev ^ 1);
